...
#		print(str(e_id)+" needs render")
		...
		var node = load(entity.renderable).instance()
		node.e_id = e_id
		node.translation = entity.needs_render
		if "player" in entity:
			node.add_child(load("res://Game/Player_Camera.tscn").instance())
		if "viewshed" in entity:
			var v_node = load("res://Game/Viewshed.tscn").instance()
